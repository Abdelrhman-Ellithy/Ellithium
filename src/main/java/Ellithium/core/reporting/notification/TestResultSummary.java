...
public class TestResultSummary {
    ...
    private final Collection<String> executedTests;
    ...
    public TestResultSummary(long totalTests, long passedTests, long failedTests, long skippedTests,
        ...
        this(totalTests, passedTests, failedTests, skippedTests, executionTime, failedTestResults,
                Collections.emptyList(), System.getProperty("ellithium.suite.name", "Test Suite"));
    }
    ...
    public TestResultSummary(long totalTests, long passedTests, long failedTests, long skippedTests,
        ...
        this(totalTests, passedTests, failedTests, skippedTests, executionTime, failedTestResults,
                executedTests, System.getProperty("ellithium.suite.name", "Test Suite"));
    }

    public TestResultSummary(long totalTests, long passedTests, long failedTests, long skippedTests,
            long executionTime, Set<ITestResult> failedTestResults,
            Collection<String> executedTests, String suiteName) {
        ...
        this.executedTests = executedTests == null ? Collections.emptyList() : executedTests;
        this.suiteName = (suiteName == null || suiteName.isBlank()) ? "Test Suite" : suiteName;
    }
    ...
    public String generateSummaryMessage() {
        try {
            ...
            message.append("🚀 *Ellithium Test Execution Summary*").append(System.lineSeparator())
                    .append(System.lineSeparator());
            message.append("*Suite:* ").append(suiteName).append(System.lineSeparator());
            message.append("*Date:* ").append(getFormattedExecutionDate()).append(System.lineSeparator());
            message.append("*Duration:* ").append(getFormattedExecutionTime()).append(System.lineSeparator())
                    .append(System.lineSeparator());
            ...
            message.append("📈 Success Rate: ").append(String.format("%.1f%%", getSuccessRate()))
            ...
            appendExecutedTestsToMessage(message);
    ...
    private void appendExecutedTestsToMessage(StringBuilder message) {
        if (executedTests.isEmpty())
            return;
        message.append("*Executed Tests:* ").append(System.lineSeparator());
        for (String test : executedTests) {
            message.append("• ").append(test).append(System.lineSeparator());
        }
    }
    ...
    private void appendFailedTestsToMessage(StringBuilder message) {
        try {
            for (ITestResult failedTest : failedTestResults) {
                ...
                String className = failedTest.getTestClass() != null ? failedTest.getTestClass().getName()
                        : "Unknown Class";
                ...
                message.append("• ").append(testName).append(" (").append(className).append(")")
                        .append(System.lineSeparator());
                message.append("  Error: ").append(errorMessage).append(System.lineSeparator())
                        .append(System.lineSeparator());
            }
        } catch (Exception e) {
    ...
    /**
     * Web-safe font stack, inlined on every element below rather than left to a
     * stylesheet.
     * Outlook desktop/MS365 (Word rendering engine) and many corporate mail
     * security gateways
     * strip or ignore {@code <head><style>} content, so anything that only lives
     * there is not
    ...
    /**
     * Generates rich HTML-formatted email content using a table-based layout with
     * every color,
     * font, and spacing inlined on the element itself, so it renders consistently
     * across email
     * clients that strip {@code <style>} blocks or ignore modern CSS. In
     * particular:
     * <ul>
     * <li>No CSS custom properties ({@code var(--x)}) — unsupported by Outlook's
     * Word engine,
     * which silently drops the whole declaration (this is what was making the
     * status badge
     * and other accent colors disappear in Outlook/MS365).</li>
     * <li>No div-based layout ({@code max-width}, {@code margin:auto}, flex/grid) —
     * layout is
     * done with {@code 
     * <table>
     * } elements, which every mail client renders consistently.</li>
     * <li>Every gradient has a solid {@code background-color} fallback declared
     * alongside it,
     * since Outlook does not paint CSS gradients at all.</li>
     * <li>Colors are set via both a {@code style} attribute and a legacy
     * {@code bgcolor}
    ...
    public String generateHtmlEmailBody() {
        try {
            ...
            html.append("<tr><td style=\"padding:28px 24px; font-family:").append(FONT_FAMILY)
                    .append("; color:#333333;\">");
            appendSummaryTable(html);
            appendProgressSection(html);
            appendExecutionInfoSection(html);
            appendExecutedTestsSection(html);
            ...
            return "<!DOCTYPE html><html><body><h1>Error generating HTML report</h1><p>" + escapeHtml(e.getMessage())
                    + "</p></body></html>";
        }
    }

    private void appendExecutedTestsSection(StringBuilder html) {
        if (executedTests.isEmpty())
            return;
        html.append("<div style=\"font-size:16px; font-weight:600; color:#495057; font-family:")
                .append(FONT_FAMILY).append("; margin:28px 0 12px;\">Executed Tests</div>");
        html.append("<ul style=\"font-family:").append(FONT_FAMILY).append("; color:#495057;\">");
        for (String test : executedTests) {
            html.append("<li>").append(escapeHtml(test)).append("</li>");
        }
        html.append("</ul>");
    }
    ...
    /**
     * Minimal progressive-enhancement styling only — a mobile-width tweak for
     * clients that honor
     * {@code <style>} blocks. Every color/spacing/font that actually matters is
     * already inlined on
     * its element by the append* methods below, so clients/gateways that strip this
     * block entirely
    ...
    private void appendHtmlBodyStart(StringBuilder html) {
        ...
        html.append(
                "<table role=\"presentation\" class=\"ellithium-wrap\" width=\"600\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\" align=\"center\" bgcolor=\"#ffffff\" style=\"width:600px; max-width:600px; background-color:#ffffff;\">");
    }

    private void appendHeaderSection(StringBuilder html, String status, String statusColor, String statusIcon) {
        html.append(
                "<tr><td align=\"center\" bgcolor=\"#5b3fa0\" style=\"background-color:#5b3fa0; background-image:linear-gradient(135deg,#667eea 0%,#764ba2 100%); padding:32px 24px; font-family:")
                .append(FONT_FAMILY).append(";\">");
        html.append(
                "<div style=\"font-size:26px; line-height:32px; font-weight:600; color:#ffffff;\">🚀 Ellithium Test Execution Report</div>");
        html.append(
                "<div style=\"font-size:14px; color:#eef0fb; margin-top:6px;\">Automated Test Results Summary</div>");
        html.append(
        ...
        html.append("<td bgcolor=\"").append(statusColor).append("\" style=\"background-color:").append(statusColor)
                ...
                .append("; font-size:14px; font-weight:bold; color:#ffffff;\">").append(statusIcon).append(" ")
    ...
    private void appendSummaryHeaderCell(StringBuilder html, String label) {
        html.append(
                "<th align=\"center\" bgcolor=\"#495057\" style=\"background-color:#495057; color:#ffffff; padding:12px; font-family:")
                .append(FONT_FAMILY).append("; font-size:12px; text-transform:uppercase; letter-spacing:1px;\">")
    ...
    private void appendSummaryRow(StringBuilder html, String label, long count, double percentage, String color) {
        ...
        html.append("<td align=\"center\" style=\"padding:16px; border-bottom:1px solid #e9ecef; font-family:")
                .append(FONT_FAMILY).append("; color:").append(color).append("; font-weight:bold; font-size:22px;\">")
    ...
    private void appendProgressSection(StringBuilder html) {
        ...
        // <td width="%">, so the bar is built from table cells rather than a CSS-width
        // div.
        ...
        if (filledPercent > 0) {
            html.append("<td width=\"").append(filledPercent).append("%\" bgcolor=\"").append(fillColor)
                    .append("\" style=\"background-color:").append(fillColor)
                    .append("; line-height:18px; font-size:1px;\">&nbsp;</td>");
        }
        ...
        html.append("<div style=\"text-align:center; font-weight:600; color:#495057; font-family:")
                ...
                .append(String.format("%.1f%% Success Rate (%d/%d tests passed)", getSuccessRate(), passedTests,
                        totalTests))
                .append("</div>");
    }
    ...
    private void appendExecutionInfoSection(StringBuilder html) {
        try {
            html.append(
                    "<table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\" bgcolor=\"#e9ecef\" style=\"background-color:#e9ecef; margin-top:24px;\">");
            html.append("<tr><td style=\"padding:20px; font-family:").append(FONT_FAMILY).append(";\">");
            html.append(
                    "<div style=\"font-size:16px; font-weight:600; color:#495057; margin-bottom:12px;\">📊 Execution Details</div>");
            html.append(
                    "<table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\">");

            appendInfoRow(html, "Suite:", suiteName);
            ...
            String statusColor = hasFailures() ? "#dc3545" : "#28a745";
            html.append(
                    "<tr><td style=\"padding:6px 0; font-weight:600; color:#495057; width:40%;\">Execution Status:</td>")
                    .append("<td style=\"padding:6px 0;\"><strong style=\"color:").append(statusColor).append(";\">")
                    .append(status).append("</strong></td></tr>");
    ...
    private void appendInfoRow(StringBuilder html, String label, String value) {
        html.append(
                "<tr><td style=\"padding:6px 0; border-bottom:1px solid #dee2e6; font-weight:600; color:#495057; width:40%;\">")
                .append(escapeHtml(label))
                .append("</td><td style=\"padding:6px 0; border-bottom:1px solid #dee2e6; color:#6c757d;\">")
    ...
    private void appendFailedTestsSection(StringBuilder html) {
        ...
        try {
            ...
            for (ITestResult failedTest : failedTestResults) {
                ...
                html.append("<tr>");
                html.append("<td style=\"padding:12px; border-bottom:1px solid #f5c6cb; font-family:")
                        .append(FONT_FAMILY)
                        .append("; font-weight:bold; color:#c53030; vertical-align:top;\">")
                        .append(escapeHtml(testName)).append("</td>");
                html.append("<td style=\"padding:12px; border-bottom:1px solid #f5c6cb; font-family:")
                        .append(FONT_FAMILY)
                        .append("; font-size:12px; color:#718096; vertical-align:top;\">").append(escapeHtml(className))
                        .append("</td>");
                html.append(
                        "<td style=\"padding:12px; border-bottom:1px solid #f5c6cb; background-color:#fdf0f0; font-family:'Courier New',Courier,monospace; font-size:12px; color:#c53030; vertical-align:top;\">")
            ...
            html.append("<div style=\"font-family:").append(FONT_FAMILY)
                    .append(";\">Error loading failed test details: ")
    ...
    private void appendFailedTableHeaderCell(StringBuilder html, String label) {
        html.append(
                "<th align=\"left\" bgcolor=\"#dc3545\" style=\"background-color:#dc3545; color:#ffffff; padding:12px; font-family:")
                .append(FONT_FAMILY).append("; font-size:13px;\">").append(label).append("</th>");
    }

    private void appendHtmlBodyEnd(StringBuilder html) {
        html.append(
                "<tr><td align=\"center\" bgcolor=\"#f8f9fa\" style=\"background-color:#f8f9fa; padding:20px; font-family:")
                .append(FONT_FAMILY).append("; font-size:12px; color:#6c757d; border-top:1px solid #e9ecef;\">");
        html.append(
                "Generated by Ellithium Test Automation Framework<br>For detailed reports, please check the Allure HTML report");
        html.append("</td></tr>");
        html.append("</table>");
        html.append("</td></tr>");
        html.append("</table>");
